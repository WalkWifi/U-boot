...
#include <spi_api.h>
#include <nand_api.h>
#include <drvlcd.h> //weikk 20160601
...
__attribute__((nomips16)) void board_init_r (gd_t *id, ulong dest_addr)
{
	...
	if(reg & RT2880_WDRST ){
		...
		printf("******************************\n");
        isSoftRst = 1;  //weikk add 20160601
...
/*config bootdelay via environment parameter: bootdelay */
    ...
    if(timer1 == 0) //weikk add 20160612,if it seted the bootdelay to zero in the environment,change it.
        timer1 = 1;
	OperationSelect();   
	while (timer1 > 0) {
		--timer1;
		...
		for (i=0; i<100; ++i) {
			if ((my_tmp = tstc()) != 0) {	/* we got a key press	*/
				...
				BootType = getc();
                //weikk 20160314,line=9,if the system gets a space key,set bootdelay=20s
                if(BootType == ' ')
                {
                    BootType = '3';
                    timer1 = 20;
	...
	if(BootType == '3') {
        lcd_main(); //weikk 20160317,detect the soc of the battery,waiting for if it's in low power.
		char *argv[2];
