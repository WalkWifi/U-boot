...
#error "undefined Platform"
    ...
    //weikk 20160719,get a accurate value.
    if(gd->baudrate > 230400)
    {
        clock_divisor = (40*1000*1000*10 / gd->baudrate);
        HSP(CFG_RT2880_CONSOLE) = 3;
    }
    else
    {
        clock_divisor = (40*1000*1000*10 / SERIAL_CLOCK_DIVISOR / gd->baudrate);
        HSP(CFG_RT2880_CONSOLE) = 0;
    }
    
    if((clock_divisor%10) >= 5)
        clock_divisor = clock_divisor/10 + 1;
    else
        clock_divisor /= 10;
	/* set baud rate */
    if(gd->baudrate > 230400)
    {
    	...
    	DLL(CFG_RT2880_CONSOLE) = 1;
    	DLM(CFG_RT2880_CONSOLE) = 0;
        SPC(CFG_RT2880_CONSOLE) = clock_divisor;
...
//weikk add 20160607 line=
//config uart0,115200/8bit/1stop/no verify. no IER
void Serial0_Init(void)
{
    unsigned int clock_divisor = (40*1000*1000/ SERIAL_CLOCK_DIVISOR / CONFIG_BAUDRATE);
    #if 1
    IER(RT2880_UART1) = 0;		/* Disable for now */
	FCR(RT2880_UART1) = 1;		/* No fifos enabled */
	LCR(RT2880_UART1) = LCR_WLS0 | LCR_WLS1 | LCR_DLAB;
	DLL(RT2880_UART1) = (clock_divisor/2) & 0xff;
	DLM(RT2880_UART1) = (clock_divisor/2) >> 8;
	LCR(RT2880_UART1) = LCR_WLS0 | LCR_WLS1;
    #endif
}
...
{
    u8 tmp = 0;
	if(LSR(RT2880_UART1) & LSR_DR){
        tmp = RBR(RT2880_UART1) & 0xff;
        //printf("read: %x \n", tmp);
    	return tmp;
	}
}

void Serial0_Send (const char *s)
{
    printf("send: %s \n", s);
	while (*s) {
        while ((LSR(RT2880_UART1) & LSR_TEMT) == 0);
