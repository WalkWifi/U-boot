...
#define CONFIG_ETH_ONE_PORT_ONLY ETH_ONE_PORT_ONLY  //weikk add 2015.10.17,in autocfg.h
    ...
    //weikk add 2015.10.20,line=7,for turnning on the power.open the green led.
    RALINK_REG(RT2880_GPIOMODE_REG) &= ~(3 << 0);   //set gpio0 to gpio mode
    RALINK_REG(RT2880_REG_PIODIR) |= (1 << 11);     //GPIO0 is GPIO#11
    RALINK_REG(RT2880_REG_PIODATA) |= (1 << 11);
    i = RALINK_REG(RT2880_GPIOMODE_REG);   //MDI_TN_P1,<GREEN LED> is GPIO#15,SPIS_MODE=01
    i = (i & (~(3 << 2))) | (1 << 2);
    RALINK_REG(RT2880_GPIOMODE_REG) = i;
    RALINK_REG(RT2880_REG_PIODIR) |= (1 << 15); 
    RALINK_REG(RT2880_REG_PIODATA) &= ~(1 << 15);
